...
class DiceView: NSView, NSDraggingSource {
    ...
    func metricsForSize(size: CGSize) -> (edgeLength: CGFloat, dieFrame: CGRect) {
        ...
        let x = (size.width - edgeLength) / 2
        let y = (size.height - edgeLength) / 2
        let drawingBounds = CGRect(x: CGFloat(x), y: CGFloat(y), width: edgeLength, height: edgeLength)
    ...
    override func drawFocusRingMask() {
        NSBezierPath.fillRect(focusRingMaskBounds)
    }
    ...
    @IBAction func savePDF(sender: AnyObject!) {
        ...
        savePanel.beginSheetModalForWindow(window!) { [unowned savePanel] result in
            if result == NSModalResponseOK {
                let data = self.dataWithPDFInsideRect(self.bounds)
                var error: NSError?
                let ok = data.writeToURL(savePanel.URL!, options:NSDataWritingOptions.DataWritingAtomic, error: &error)
                if !ok {
